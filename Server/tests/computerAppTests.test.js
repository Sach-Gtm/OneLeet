// Verifies the Computer Application batch: 4 chapters (Computer Fundamentals,
// Number Systems, Software & Operating Systems, Networking & Internet), each in
// BOTH a repeatable practice and a single-attempt graded "test" flavour, with
// DIFFERENT questions per flavour. 8 sets × 10 Q = 80. Quick Shot, 25-min, all
// exams, idempotent.
// Run: node tests/computerAppTests.test.js
...
const { TESTS, ensureComputerAppTestsSeeded } = require("../src/config/seedComputerAppTests");
...
(async () => {
    ...
    const chapters = ["Computer Fundamentals", "Number Systems", "Software & Operating Systems", "Networking & Internet"];
    chapters.forEach((c) => {
    ...
    await ensureComputerAppTestsSeeded();
    for (const t of TESTS) {
        ...
        assert.strictEqual(doc.subject, "Computer Application", `${t.slug} subject is Computer Application`);
    ...
    ok("eight sets seed with the right mode, 25-min, Computer Application, all exams");

    const titles = TESTS.map((t) => t.title);
    const beforeQ = await Question.countDocuments({ subject: "Computer Application" });
    await ensureComputerAppTestsSeeded();
    assert.strictEqual(await Test.countDocuments({ title: { $in: titles } }), 8, "no duplicate tests on re-seed");
    assert.strictEqual(await Question.countDocuments({ subject: "Computer Application" }), beforeQ, "no duplicate questions on re-seed");
    ok("re-seeding is a no-op");

    await mongoose.disconnect();
    await mongod.stop();
    console.log(`\n✅ All ${passed} computer-application checks passed`);
    process.exit(0);
})().catch(async (e) => {
    console.error("\n❌ COMPUTER APP TEST FAILED:", e.message);
