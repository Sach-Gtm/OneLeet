...
async function buildPillar() {
    const faqs = [
        ...
        { q: "Who is eligible for LEET / lateral entry?", a: `Generally, candidates with a three-year engineering diploma (typically around 45 to 50% marks, often relaxed for reserved categories) are eligible; several states also admit B.Sc graduates with Mathematics. Exact eligibility varies by state and year, so confirm the official notification.` },
...
${ctaBlock(`Practise ${sub.name} for ${s.name} LEET`, "Unlimited AI questions, real past papers and timed mocks, all free.")}
...
<p>Begin with <a href="${BASE}/leet/subjects/mathematics/">Mathematics</a>, which carries the most weight in almost every state, then layer in Physics and Chemistry. Follow the full sequence in our <a href="${BASE}/leet/how-to-prepare/">study plan</a>.</p></section>`;
...
async function buildTopic(sub, topic) {
    ...
    const faqs = [
        ...
        { q: `How do I master ${topic} quickly?`, a: `Understand the core idea, memorise the standard formulae, then drill 20 to 30 varied problems. OneLeet's AI can generate unlimited ${topic} questions at your level.` },
    ];
    const f = faq(faqs);
    const body = `
...
${ctaBlock(`Practise ${topic} now`, "Generate unlimited questions on this exact topic with OneLeet's AI, then test yourself with timed mocks, all free.")}
...
${ctaBlock(`Prepare to crack lateral entry`, "The seat goes to the rank. Build yours with real papers, mocks and AI practice, all free.")}
...
<li><strong>Minimum marks:</strong> usually around 45 to 50% aggregate, with a relaxation (often ~5%) for reserved categories. The exact figure is set by each state.</li>
...
${ctaBlock()}`,
            faqs: [
                { q: "What percentage is needed for lateral entry?", a: "Most states ask for roughly 45 to 50% in your diploma, relaxed for reserved categories. Confirm your state's exact cut-off in its notification." },
...
<li><strong>Duration:</strong> commonly around 90 to 120 minutes.</li>
...
${ctaBlock()}`,
            faqs: [
                ...
                { q: "How long is the LEET exam?", a: "Typically around 90 to 120 minutes of objective questions, though this varies by state." },
            ],
        },
        {
...
<section><h2>More guides</h2>${cards(GUIDES.slice(2, 8), guideCard)}</section>`,
            faqs: [
                ...
                { q: "How many months of preparation does LEET need?", a: "It varies, but 3 to 6 focused months alongside your diploma is a realistic target for most aspirants." },
            ],
        },
        {
...
<section><h2>Format</h2><p>Objective MCQs, Maths-heavy, usually 90 to 120 minutes. Negative marking varies by state. Full detail: <a href="${BASE}/leet/exam-pattern/">exam pattern</a>.</p></section>
...
<section><h2>Weeks 4 to 3</h2><p>Revise high-yield <a href="${BASE}/leet/subjects/mathematics/">Maths</a> and Physics topics; one <a href="${BASE}/leet/mock-tests/">mock</a> mid-week, analysed fully.</p></section>
<section><h2>Weeks 2 to 1</h2><p>Two mocks a week under exam conditions. Convert every repeat mistake into a revision card. Sharpen speed and accuracy. No new topics.</p></section>
