...
export default function TestsList() {
    ...
    const [limit, setLimit] = useState(9); // how many cards to render (Show more grows it)
    ...
    // Reset the visible count whenever the view or a filter changes, so switching
    // never lands you deep in a long "Show more"-expanded list.
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional reset on view/filter change
    useEffect(() => setLimit(9), [view, fFormat, fSubject, fCollege, fChapter]);
    ...
    const activeFilterCount = [fFormat, fSubject, fCollege, fChapter].filter((v) => v !== "all").length;
    const clearFilters = () => {
        setFFormat("all");
        setFSubject("all");
        setFCollege("all");
                    ...
                    <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            >
                                ...
                                {activeFilterCount > 0 && (
                                    <span className="grid h-5 min-w-5 place-items-center rounded-full bg-indigo-600 px-1 text-xs font-bold text-white">{activeFilterCount}</span>
                                )}
                        ...
                        {showFilters && (
                            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                                ...
                                {subjectOptions.length <= 1 && formatOptions.length <= 1 && collegeOptions.length <= 1 && chapterOptions.length <= 1 && (
                                    <span className="px-1 text-xs text-slate-400">No extra filters for this view.</span>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    {shown.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            <p className="text-xs text-slate-400">
                                Showing {Math.min(limit, shown.length)} of {shown.length} test{shown.length === 1 ? "" : "s"}
                            </p>
